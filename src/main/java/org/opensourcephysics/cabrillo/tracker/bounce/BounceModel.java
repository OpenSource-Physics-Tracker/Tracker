 ...
 * <p>
 * For handling 2D data, the parameters are a numParams x 2 Matrix.
 * <p>
...
public class BounceModel {
    ...
    public double getStepAt(BounceMatrix model_param) {
        if (!useUnknownStep) return stepAt;
        ...
        // get weighted average of extra time past stepAt
        double weight = 0;
        for (int dim = 0; dim < dimension; dim++) {
            double dv = param_array[degree + 1][dim];    // estimate of the velocity step size
            double x = param_array[degree + 2][dim];    // estimate of how much more displacement there is than if the step were at stepAt
            if (dv == 0) continue;
            guess_step += dv * x;    // add extra_t * dv^2
            weight += dv * dv;    // dv^2
        }
        if (weight > 0) guess_step /= weight;
        return stepAt - guess_step;
    }
    ...
    public BounceParameters fit_xy(double[] xData, double[] yData, int start, int index_step) {
        ...
        BounceMatrix params = inverseModel.times(data_matrix);
        ...
        for (int dim = 0; dim < 2; dim++) {
            ...
            double extra = params_array[degree + 2][dim];    // estimate of how much more displacement there is than if the step were at stepAt
            if (dv == 0) continue;
     ...
     */
    public BounceParameters fit_xy(double[] xData, double[] yData, int start, int index_step,
                                   double initial_stepAt, double[] initial_step_size
    ) {
        if (useUnknownStep || (useStep && stepAt != initial_stepAt)) {
        ...
        return new BounceParameters(this, params, square_error, initial_stepAt, initial_step_size);
    }
