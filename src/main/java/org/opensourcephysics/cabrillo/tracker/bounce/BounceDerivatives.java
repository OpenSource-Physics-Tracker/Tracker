...
public class BounceDerivatives implements Derivative {
    ...
    public Object[] evaluate(Object[] data) {
        ...
        for (int n = 0; n < length; n++) {    // mark all the outputs as invalid
        ...
        if (start >= length) {    // this was a dummy call, probably with initial (empty) data arrays
            return result;
        }
        ...
        BounceModel poly_model = new BounceModel(window_size, degree, 0);
        ...
        BounceModel step_model = new BounceModel(window_size, degree, Double.NaN);
        ...
        for (int c = 0; c < count; c++) {
            ...
            if (highest_bad_index >= 0) {
                ...
                c_at[c] -= (move_up <= move_down) ? move_up : (-move_down);
        ...
        for (int c = 0; c < count; c++) {
            ...
            for (int i_wind = Math.max(start, (int) (i_step_time - 0.5 * (window_size - 1) + 0.999));
                ...
                double poly_err = poly_fit[c_wind].getSquareError();
                ...
                double step_err = refit.getSquareError();
        ...
        Arrays.sort(best_step_locs, (o1, o2) -> Double.compare(step_value[o2], step_value[o1]));
        ...
        for (int k = 0; k < count; k++) {
            ...
            double poly_error = poly_fit[c].getSquareError();

            final double what_is_large = 0.6 * window_size;
            if (step_value[c] < what_is_large * poly_error) {   // step is not big, get rid of it
                continue;
            }
        ...
        for (int c = 0; c < count; c++) {
            ...
            if (null == poly_fit[c]) {   // couldn't fit any models here
                continue;
            }
