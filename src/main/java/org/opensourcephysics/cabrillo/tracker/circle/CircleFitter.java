 ...
 */
package org.opensourcephysics.cabrillo.tracker.circle;
...
import org.opensourcephysics.cabrillo.tracker.dialog.AttachmentDialog;
...
import org.opensourcephysics.cabrillo.tracker.circle.CircleFitterStep.DataPoint;
import org.opensourcephysics.cabrillo.tracker.circle.CircleFitterStep.CenterPoint;
...
public class CircleFitter extends TTrack {
    ...
    public static int maxDataPointCount = 50;
    ...
    public boolean attachToSteps = false, isRelativeFrameNumbers = false;
    public int absoluteStart = 0, relativeStart = -2, attachmentFrameCount = 5;
    ...
    public CircleFitter() {
        ...
        originToCenterItem.addActionListener(e -> setCoordsOriginToCenter());
        ...
        pointCountButton = new TButton() {
            protected JPopupMenu getPopup() {
                ...
                buf.append(step.getTrack().getName()).append("_").append(stepLabel.getText()).append("_").append(trackerPanel.getStepNumber()); //$NON-NLS-1$ //$NON-NLS-2$
                buf.append(XML.NEW_LINE);
                buf.append("x").append(TrackerIO.getDelimiter()).append("y"); //$NON-NLS-1$ //$NON-NLS-2$
                buf.append(XML.NEW_LINE);
                for (int i = 0; i < pts.size(); i++) {
                    ...
                    buf.append(formattedX).append(TrackerIO.getDelimiter()).append(formattedY);
                    if (i < pts.size() - 1) {
                        buf.append(XML.NEW_LINE);
                    }
    ...
    public void dispose() {
        ...
        if (attachmentForSteps != null) {
            Arrays.fill(attachmentForSteps, null);
        }
        if (attachments != null) {
            ...
            public void run() {
                ...
                for (TTrack attachment : attachments) {
                    if (attachment != null) {
                        hasAttachments = true;
                        // refresh listeners
                        attachment.removePropertyChangeListener("step", CircleFitter.this); //$NON-NLS-1$
                        attachment.removePropertyChangeListener("steps", CircleFitter.this); //$NON-NLS-1$
                        attachment.addPropertyChangeListener("step", CircleFitter.this); //$NON-NLS-1$
                        attachment.addPropertyChangeListener("steps", CircleFitter.this); //$NON-NLS-1$
                    }
                }
                if (hasAttachments) {
                ...
                TreeSet<Integer> framesToRefresh = new TreeSet<>();
                if (!attachToSteps) {
                    ...
                    if (targetTrack != null) {
                        ...
                        for (int stepNum = 0; stepNum <= stepCount; stepNum++) {
                            ...
                            boolean noPoints = in >= out && isNoPoints(n);
    ...
    static class Loader implements XML.ObjectLoader {
        ...
        public Object loadObject(XMLControl control, Object obj) {
            ...
            for (double[] datum : data) {
                if (datum == null || datum.length < 1) continue;
                ...
                if (datum.length == 1) { // no data points
                ...
                int pointCount = version < 4.92 ? (datum.length - 3) / 2 : (datum.length - 1) / 2;
                DataPoint[] loadedPoints = new DataPoint[pointCount];
                for (int j = 0; j < pointCount; j++) {
                    loadedPoints[j] = step.new DataPoint(datum[2 * j + 1], datum[2 * j + 2]);
                }
