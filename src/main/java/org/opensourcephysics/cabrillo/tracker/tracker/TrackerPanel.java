 ...
 */
package org.opensourcephysics.cabrillo.tracker.tracker;
...
import org.opensourcephysics.cabrillo.tracker.configuration.Configuration;
import org.opensourcephysics.cabrillo.tracker.coord.CoordAxes;
import org.opensourcephysics.cabrillo.tracker.dialog.*;
...
import org.opensourcephysics.cabrillo.tracker.line.LineProfileStep;
import org.opensourcephysics.cabrillo.tracker.model.ModelBuilder;
...
import org.opensourcephysics.cabrillo.tracker.plot.PlotGuestDialog;
...
import org.opensourcephysics.cabrillo.tracker.track.*;
...
public class TrackerPanel extends VideoPanel implements Scrollable {
    ...
    public static String alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; //$NON-NLS-1$

    // instance fields
    protected double defaultImageBorder;
    protected String description = ""; //$NON-NLS-1$
    public TPoint selectedPoint;
    public Step selectedStep;
    public TrackerPanel selectingPanel;
    public TTrack selectedTrack;
    public TPoint newlyMarkedPoint;
    ...
    public TPoint pointState = new TPoint();
    public MouseEvent mEvent;
    public TMouseHandler mouseHandler;
    protected JLabel badNameLabel = new JLabel();
    public TrackDataBuilder dataBuilder;
    public boolean dataToolVisible;
    public XMLProperty viewsProperty; // TFrame loads views
    public XMLProperty selectedViewsProperty; // TFrame sets selected views
    public double[] dividerLocs; // TFrame sets dividers
    protected Point zoomCenter; // used when loading
    public Map<Filter, Point> visibleFilters; // TFrame sets locations of filter inspectors
    public int trackControlX = Integer.MIN_VALUE, trackControlY; // TFrame sets track control location
    public int infoX = Integer.MIN_VALUE, infoY; // TFrame sets info dialog location
    public JPanel noData = new JPanel();
    protected JLabel[] noDataLabels = new JLabel[2];
    public boolean isEmpty;
    public String defaultSavePath, openedFromPath;
    public ModelBuilder modelBuilder;
    public TrackControl trackControl;
    public boolean isModelBuilderVisible;
    public boolean isShiftKeyDown, isControlKeyDown;
    public ArrayList<TTrack> calibrationTools = new ArrayList<>();
    public Set<TTrack> visibleTools = new HashSet<>();
    public String author, contact;
    public AutoTracker autoTracker;
    protected DerivativeAlgorithmDialog algorithmDialog;
    public AttachmentDialog attachmentDialog;
    protected PlotGuestDialog guestsDialog;
    protected UnitsDialog unitsDialog;
    public boolean isAutoRefresh = true;
    public TreeSet<String> supplementalFilePaths = new TreeSet<>(); // HTML/PDF URI paths
    public Map<String, String> pageViewFilePaths = new HashMap<>();
    public StepSet selectedSteps = new StepSet(this);
    public boolean hideDescriptionWhenLoaded;
    public PropertyChangeListener massParamListener, massChangeListener;
    public Map<Class<? extends TTrack>, TreeMap<String, String>> formatPatterns
            = new HashMap<>();
    public String lengthUnit = "m", massUnit = "kg", timeUnit = "s"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    protected boolean unitsVisible = true; // visible by default
    public TCoordinateStringBuilder coordStringBuilder;
     ...
     */
    public FunctionPanel createFunctionPanel(TTrack track) {
        ...
        else if (org.opensourcephysics.cabrillo.tracker.vector.Vector.class.isAssignableFrom(type))
            panel.setDescription(org.opensourcephysics.cabrillo.tracker.vector.Vector.class.getName());
...
//    if (modelBuilder != null) modelBuilder.removePanel(track.getName());
    ...
    public String getUnits(TTrack track, String var) {
        ...
        if (dimensions.equals("ML/T"))
            return sp + massUnit + d + lengthUnit + "/" + timeUnit; //$NON-NLS-1$ //$NON-NLS-2$
        if (dimensions.equals("ML/TT"))
            return sp + massUnit + d + lengthUnit + "/" + timeUnit + sq; //$NON-NLS-1$ //$NON-NLS-2$
        if (dimensions.equals("MLL/TT"))
            return sp + massUnit + d + lengthUnit + sq + "/" + timeUnit + sq; //$NON-NLS-1$ //$NON-NLS-2$
        TFrame frame = getTFrame();
     ...
     */
    public DataTrack importData(String dataString, Object source) {
        if (dataString == null) {
            // inform user
        ...
        return importData(ResourceLoader.getString(dataString), dataString);
    }
    ...
    public void refreshDecimalSeparators() {
        ...
        for (org.opensourcephysics.cabrillo.tracker.protractor.Protractor p : getDrawables(Protractor.class)) {
     ...
     */
    public int getAlphabetIndex(String name, String connector) {
                ...
                if (e.getKeyCode() == KeyEvent.VK_SHIFT) {
                    ...
                    if (step != null) {
                        ...
                        if (selectedTrack.getClass() == PointMass.class) {
                            ...
                            selectedTrack.keyFrames.add(n);
                        } else if (selectedTrack.getClass() == org.opensourcephysics.cabrillo.tracker.vector.Vector.class) {
                            VectorStep s = (VectorStep) step;
                            TPoint tail = s.getTail();
                            TPoint tip = s.getTip();
                            org.opensourcephysics.cabrillo.tracker.vector.Vector vector = (org.opensourcephysics.cabrillo.tracker.vector.Vector) selectedTrack;
     ...
     */
    public boolean setCursorForMarking(boolean invert, InputEvent e) {
        ...
        if (marking) {
            ...
            if (Tracker.showHints) {
                ...
                } else if (selectedTrack instanceof org.opensourcephysics.cabrillo.tracker.vector.Vector)
                    if (selectedTrack.getStep(n) == null)
    ...
    public Set<String> getEnabled() {
        if (enabled == null) enabled = new TreeSet<>();
        return enabled;
    }
    ...
    public void propertyChange(PropertyChangeEvent e) {
        ...
        if (name.equals("step") || name.equals("steps")) { // from tracks/steps //$NON-NLS-1$ //$NON-NLS-2$
            ...
            if (getVideo() != null && !getVideo().getFilterStack().isEmpty()) {
                for (Object next : getVideo().getFilterStack().getFilters()) {
            ...
            else firePropertyChange("adjusting", null, e.getNewValue()); // to particle models //$NON-NLS-1$
            if (getSelectedPoint() != null) {
                getSelectedPoint().showCoordinates(this);
                TFrame frame = getTFrame();
...
//    }
            ...
            dataToolVisible = (Boolean) e.getNewValue();
            ...
            switch (name) {
                case "stepbutton":  //$NON-NLS-1$
                    if (!(Boolean) e.getNewValue()) setMessage(""); //$NON-NLS-1$
    ...
    public static void main(String[] args) {
        ...
        for (Frame value : frames) {
            if (value.getName().equals("LauncherTool")) { //$NON-NLS-1$
                launcherFrame = value;
                break;
            }
        }
...
//    Tracker.updateResources();
     ...
     */
    public void deletePoint(TPoint pt) {
        for (TTrack track : getTracks()) {
...
//    Tracker.logTime(getClass().getSimpleName()+hashCode()+" painting"); //$NON-NLS-1$
     ...
     */
    public TreeMap<String, String> getFormatPatterns(Class<? extends TTrack> trackType) {
     ...
     */
    public void setTrackName(TTrack track, String newName, boolean postEdit) {
...
//      control.setValue("version", Tracker.VERSION); //$NON-NLS-1$
            ...
            if (frame != null) {
                ...
                for (TView[] customView : customViews) {
                ...
                for (String selectedView : selectedViews) {
            ...
            // save DataTool tabs
            ArrayList<DataToolTab> tabs = new ArrayList<>();
            ...
            if (!tabs.isEmpty()) {
                DataToolTab[] tabArray = tabs.toArray(new DataToolTab[0]);
...
//        }
            ...
            // load the tracks
            ArrayList<?> tracks = (ArrayList) control.getObject("tracks"); //$NON-NLS-1$
            if (tracks != null) {
