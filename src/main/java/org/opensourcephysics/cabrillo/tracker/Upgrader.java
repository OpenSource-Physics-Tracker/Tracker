...
public class Upgrader {
    ...
    private void upgradeWindows(final boolean[] failed) {
        ...
        final String upgradeInstallerURL = Objects.requireNonNull(upgradeURL).trim() + upgradeInstallerName;
        int responseCode = 0;
        try {
        ...
        if (responseCode == 200) { // upgrade installer exists
            ...
            Runnable runner = () -> {
                ...
                try {
                    ...
                    HttpURLConnection huc = (HttpURLConnection) url.openConnection();
                    responseCode1 = huc.getResponseCode();
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
    ...
    private void upgradeOSX(final boolean[] failed) {
        ...
        final String zipURL = Objects.requireNonNull(upgradeURL).trim() + zipFileName;
        int responseCode = 0;
        try {
    ...
    private void upgradeLinux(final boolean[] failed) {
        ...
        final String fileURL = Objects.requireNonNull(upgradeURL).trim() + upgradeFileName;
        int responseCode = 0;
        try {
