...
public class TrackerStarter {
    ...
    private static final FilenameFilter trackerJarFilter = new TrackerJarFilter();

    private static boolean launching;
    private static boolean relaunching;
    private static boolean abortExit;
    ...
    private static int preferredMemorySize;

    private static File codeBaseDir;

    private static String newline = "\n"; //$NON-NLS-1$
    private static String exceptions = ""; //$NON-NLS-1$
    private static String starterWarning;
    private static String trackerHome;
    private static String javaHome;
    private static String ffmpegHome;
    private static String userDocuments;
    private static String startLogPath;
    private static String launchVersionString;
    private static String trackerJarPath;
    private static String logText = ""; //$NON-NLS-1$
    private static String javaCommand = "java"; //$NON-NLS-1$
    private static String bundledVM;
    ...
    static {
        ...
        try {
            ...
            File starterJarFile = new File(uri);
    ...
    public static void launchTracker(String[] args) {
        ...
        StringBuilder argString = null;
        if (args != null && args.length > 0) {
            argString = new StringBuilder(); //$NON-NLS-1$
            for (String next : args) {
                argString.append("\"").append(next).append("\" "); //$NON-NLS-1$ //$NON-NLS-2$
            }
        }
    ...
    public static XMLControl findPreferences() {
        ...
        Map<File, XMLControl> controls = new HashMap<>();
     ...
     */
    public static String findFFMPegHome(String trackerHome, boolean writeToLog) {
    ...
    private static void loadPreferences() {
        ...
        if (!prefsXMLControl.failedToRead()) {
            ...
            if (prefsXMLControl.getPropertyNames().contains("run")) { //$NON-NLS-1$
                loaded = true;
                String[] executables = (String[]) prefsXMLControl.getObject("run"); //$NON-NLS-1$
                for (String app : executables) {
                    if (app == null)
                        continue;
     ...
     */
    private static String getTrackerJarPath() {
        ...
        try {
            ...
            if (fileNames != null && fileNames.length > 0) {
                StringBuilder sBuilder = new StringBuilder("tracker jars found: ");//$NON-NLS-1$
                for (String next : fileNames) {
                    sBuilder.append(next).append(", "); //$NON-NLS-1$
                }
                ...
                for (String fileName : fileNames) {
                    ...
                    try {
                        String vers = fileName.substring(8);
    ...
    private static void startTracker(String jarPath, String[] args)
        ...
        if (memorySize < preferredMemorySize) {
            ...
            logMessage("setting environment variable MEMORY_SIZE = " + memorySize); //$NON-NLS-1$
        } else {
            env.remove("MEMORY_SIZE"); //$NON-NLS-1$
        }
        ...
        if (starterWarning != null) {
            ...
            env.remove("STARTER_WARNING"); //$NON-NLS-1$
        }
        ...
        for (String next : cmd) {
            message.append(next).append(" "); //$NON-NLS-1$
        }
        ...
        if (result > 0) {
            ...
            try {
                ...
                s = reader.readLine();
                StringBuilder errorsBuilder1 = new StringBuilder(errors);
                while (s != null && s.length() > 0) {
                    errorsBuilder1.append("\n      ").append(s); //$NON-NLS-1$
                    s = reader.readLine();
                }
                errors = errorsBuilder1.toString();
                reader.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            ...
            if (errors.contains("heap")) { //$NON-NLS-1$
                memorySize *= 0.95;
                if (memorySize < 64) {
    ...
    public static void setTrackerHome(String trackerHome) {
        TrackerStarter.trackerHome = trackerHome;
    }
}